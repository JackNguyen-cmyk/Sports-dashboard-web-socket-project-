...
**And it cannot be overridden from inside a test file.** `ws/arcjet.js` reads
`ARCJET_ENABLED` once at import. A test that does
`process.env.ARCJET_ENABLED = 'false'` on line 1 and then imports `app.js` has
already lost: ESM hoists every `import` above the file's own statements, so
`arcjet.js` evaluated — and built two live Arcjet clients from the key in
`.env` — before line 1 ran. Verified with a probe file: flag set inside the
file, `httpArcjet live? true`; flag set in the shell before `node` started,
`false`. The override has to happen outside the module graph, so `npm test` is
now `ARCJET_ENABLED=false node --test`. Found on 2026-09-12 while planning route
tests, before any had been written — the WS tests had been silently building
live clients on every run and only got away with it because they inject a fake.
...
**Scale the product, not the demo.** After the load-test baseline produced
interesting leads — a 3× write-latency jump, a measured fan-out cost — the
roadmap put Redis pub/sub, backpressure and a 5,000-connection test ahead of
deploy, auth and live data. Meanwhile the app could not update a score, and a
match created as `scheduled` stayed `scheduled` forever: `getMatchStatus` was
called once at insert and `syncMatchStatus`, the function written to fix that,
was called by nothing but its own tests. Neither gap was on the roadmap at all.
Interesting measurements pull attention toward optimising the path that exists
and away from noticing the path that does not. The check: before scaling
anything, list what the product is *for* and confirm each item can actually
happen. Reordered 2026-09-12; see `ROADMAP.md` stage 3.
