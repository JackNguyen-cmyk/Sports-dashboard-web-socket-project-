...
/**
 * `db` is a parameter, not an import: importing db/db.js opens a real pg Pool
 * and throws without DATABASE_URL, so a router that imports it can only be
 * tested against a live database. Handed in, the same code runs against a
 * fake that returns canned rows or throws on demand.
 */
