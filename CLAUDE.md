...
**1. Error responses leak database internals.** `routes/matches.js` sends
...
**2. Zod errors are unreadable.** `JSON.stringify(parsed.error)` produces an
...
**3. No graceful shutdown.** On SIGTERM/SIGINT the pg pool and ws server are
dropped mid-flight. Close server, then `wss.close()`, then `pool.end()`.

**4. Unused schemas.** `matchIdParamSchema` and `updateScoreSchema` exist with
no endpoints — no `GET /matches/:id`, no score update. `commentary` is also
still an empty table with no code touching it.

**5. No route tests.** Only `utils/match-status.js` is tested. Routes have the
most branching. `node:test` + `fetch` against `app.listen(0)` (port 0 = any
free port) covers 400/201/500 without new dependencies.

**6. `.idea/` is not in the root `.gitignore`.** JetBrains config at the repo
root is unignored. One line fixes it.

**7. Express 5 forwards async errors automatically** (verified) — unlike
